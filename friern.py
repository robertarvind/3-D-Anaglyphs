...
picrott = cv2.imread('imagesn1.tif')
picroott = cv2.imread('imagesn1.tif')
...
ccl1 = cclahe1.apply(pic1)
gray1 = cv2.GaussianBlur(ccl1, (41, 41), 0)
(minVal1, maxVal1, minLoc1, maxLoc1) = cv2.minMaxLoc(gray1)
piic1 = pic1#[maxLoc1[1]-200:maxLoc1[1]+200, maxLoc1[0]-200:maxLoc1[0]+200]
pix1 = picg#[maxLoc1[1]-200:maxLoc1[1]+200, maxLoc1[0]-200:maxLoc1[0]+200]
...
ppic1 = cv2.imread('imagesn2.tif')
ppiclol1 = cv2.imread('imagesn2.tif')
pix2 = ppic1
pixrot = ppiclol1
pixrot[:,:,0]=0
pixrot[:,:,1]=0
...
for angle in range(0, 360, 1):
  ...
  ccl2 = cclahe2.apply(rotated)
  gray1 = cv2.GaussianBlur(ccl2, (41, 41), 0)
  (minVal, maxVal, minLoc, maxLoc) = cv2.minMaxLoc(gray1)
  ppicc11 = rotated#[maxLoc[1]-200:maxLoc[1]+200, maxLoc[0]-200:maxLoc[0]+200]
...
for angle in range(0, 360 - angles[znccpt] , 1):
  ...
  cl1 = clahe1.apply(rotatedd)
  grayy1 = cv2.GaussianBlur(cl1, (41, 41), 0)
  (minVall1, maxVall1, minLocc1, maxLocc1) = cv2.minMaxLoc(grayy1)
  rotatedd = rotatedd#[maxLocc1[1]-200:maxLocc1[1]+200, maxLocc1[0]-200:maxLocc1[0]+200]
  rrotatedd = rrotatedd#[maxLocc1[1]-200:maxLocc1[1]+200, maxLocc1[0]-200:maxLocc1[0]+200]
...
picx3d = cv2.resize(pic3d,None,fx=1, fy=1, interpolation = cv2.INTER_CUBIC)
for angle in range(0, 360 - angles[znccpt] , 1):
  rotatedd3d = imutils.rotate(pic3d, angle=angle)
  picc3d = cv2.resize(rotatedd3d,None,fx=1, fy=1, interpolation = cv2.INTER_CUBIC)
for angle in range(0, 90, 1):
  rotatedd3d1 = imutils.rotate(pic3d, angle=angle)
  picc3d1 = cv2.resize(rotatedd3d1,None,fx=1, fy=1, interpolation = cv2.INTER_CUBIC)
for angle in range(0, 90, 1):
  rotatedd3d2 = imutils.rotate(rotatedd3d, angle=angle)
  picc3d2 = cv2.resize(rotatedd3d2,None,fx=1, fy=1, interpolation = cv2.INTER_CUBIC)
for angle in range(0, 180, 1):
  rotatedd3dx1 = imutils.rotate(pic3d, angle=angle)
  picc3dx1 = cv2.resize(rotatedd3dx1,None,fx=1, fy=1, interpolation = cv2.INTER_CUBIC)
for angle in range(0, 180, 1):
  rotatedd3dy2 = imutils.rotate(rotatedd3d, angle=angle)
  picc3dy2 = cv2.resize(rotatedd3dy2,None,fx=1, fy=1, interpolation = cv2.INTER_CUBIC)
